    ...
    $("#search").submit(function(event) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            // 시, 도 및 시, 군, 구에 해당되는 병원 정보 출력
            var searchCity = response.searchCity;
            var possHosp = response.possHosp;

            var positions = [];  // 지도 마커 위치 지정
            var markersColor = [];  // 지도 마커 색 지정

            var idxList = [];

            // 업데이트 시간
            updateTime = possHosp[0].hvidate;
            year = updateTime.slice(0, 4);
            month = updateTime.slice(4, 6);
            day = updateTime.slice(6, 8);
            hour = updateTime.slice(8, 10);
            ...
            $("#updateTime").append($('<p style="color: grey; margin: 0 0 0 0;">').text(updateWrite));
            $("#updateTime").append($('<hr style="color: gray; margin-top: 0; margin-bottom: 0;">'));

            for(var i = 0; i<possHosp.length; i++){
                // table
                dutyname = possHosp[i].dutyname;
                dutyaddr = searchCity[i].dutyaddr;
                dutytel = possHosp[i].dutytel;
                hvec = possHosp[i].hvec;
                hv28 = possHosp[i].hv28;
                idx = possHosp[i].idx;

                idxList.push(idx);

                var form = $('<form action="/hosp" class="detailForm" style="margin-top: 0.3rem; margin-bottom: 0.3rem;">');
                var table = $('<table>');
                ...
                if (parseInt(hvec) <= 0) {
                    thtr.append($('<td class="CirTd">').append($('<img src="/img/cir_r.png" style="width: 12px; height: 12px; margin-bottom: 3px;">')));
                } else if (parseInt(hvec) <= 4) {
                    thtr.append($('<td class="CirTd">').append($('<img src="/img/cir_o.png" style="width: 12px; height: 12px; margin-bottom: 3px;">')));
                } else if (parseInt(hvec) <= 9) {
                    thtr.append($('<td class="CirTd">').append($('<img src="/img/cir_y.png" style="width: 12px; height: 12px; margin-bottom: 3px;">')));
                } else if (hvec == "-") {
                    thtr.append($('<td class="CirTd">').append($('<img src="/img/cir_r.png" style="width: 12px; height: 12px; margin-bottom: 3px;">')));
                }  else {
                    thtr.append($('<td class="CirTd">').append($('<img src="/img/cir_g.png" style="width: 12px; height: 12px; margin-bottom: 3px;">')));
                }

                thtr.append($('<td class="EmerCntTd" style="width: 100%; font-size: 0.9rem;">').text('응급실 가용병상 (' + hvec + '명)'));
                thtr.append($('<td style="float: right">').append($('<input type="submit" class="btn btn-outline-secondary" value="상세보기" style="padding-top: 1px; padding-bottom: 1px;">')));
                thead.append(thtr);
                table.append(thead);
                ...
                if(hv28 == "-") {
                    tbtr1.append($('<td class="NameTd" style="font-size: 1.1rem;" colspan="3">').text(dutyname));
                } else if(parseInt(hv28) <= 0) {
                    var kidImage = $('<img style="height: 30px;">').attr('src', '/img/kid_r.png');
                    tbtr1.append($('<td class="NameTd" style="font-size: 1.1rem;" colspan="2">').text(dutyname));
                    tbtr1.append($('<td class="ImgTd" colspan="1" style="text-align: center;">').append(kidImage));
                } else if(parseInt(hv28) <= 4) {
                    var kidImage = $('<img style="height: 30px;">').attr('src', '/img/kid_o.png');
                    tbtr1.append($('<td class="NameTd" style="font-size: 1.1rem;" colspan="2">').text(dutyname));
                    tbtr1.append($('<td class="ImgTd" colspan="1" style="text-align: center;">').append(kidImage));
                } else if(parseInt(hv28) <= 9) {
                    var kidImage = $('<img style="height: 30px;">').attr('src', '/img/kid_y.png');
                    tbtr1.append($('<td class="NameTd" style="font-size: 1.1rem;" colspan="2">').text(dutyname));
                    tbtr1.append($('<td class="ImgTd" colspan="1" style="text-align: center;">').append(kidImage));
                } else {
                    var kidImage = $('<img style="height: 30px;">').attr('src', '/img/kid_g.png');
                    tbtr1.append($('<td class="NameTd" style="font-size: 1.1rem;" colspan="2">').text(dutyname));
                    tbtr1.append($('<td class="ImgTd" colspan="1" style="text-align: center;">').append(kidImage));
                }

                tbtr2.append($('<td class="AddrTd" colspan="3">').text(dutyaddr));
                tbtr3.append($('<td class="TelTd" colspan="3">').text('응급실 연락처 : ' + dutytel));
                ...
                $("#hospInfo").append(form);
                $("#hospInfo").append($('<hr style="margin-top: 0.3rem; margin-bottom: 0.3rem;">'));

                var postForm = $('<input type="hidden">')
                $("#hospInfo").append();

                // 지도 마커
                wgslat = parseFloat(searchCity[i].wgslat);
                wgslon = parseFloat(searchCity[i].wgslon);

                positions.push({content: '<div>' + dutyname + '</div>', latlng: new kakao.maps.LatLng(wgslat, wgslon)});
                markersColor.push(parseInt(hvec));
            }
            $("#hospInfo").css('overflow-x', 'hidden');
            $("#hospInfo").css('overflow-y', 'scroll');
            ...
            for (var i = 0; i < positions.length; i ++) {
                ...
                kakao.maps.event.addListener(marker, 'click', markerClick(idxList[i]));
            ...
            var HospLoc = new kakao.maps.LatLng(wgslat, wgslon);
            map.setCenter(HospLoc);
            map.setLevel(7);
    ...
    $("#nameSearch").submit(function(event) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            $("#updateTime").append($('<p style="color: gray; margin: 0 0 0 0;">').text(updateWrite));
            $("#updateTime").append($('<hr style="color: gray; margin-top: 0; margin-bottom: 0;">'));
            ...
            for(var i = 0; i<possHosp.length; i++){
                ...
                thtr.append($('<td class="EmerCntTd" style="width: 100%; font-size: 0.9rem;">').text('응급실 가용병상 (' + hvec + '명)'));
                thtr.append($('<td style="float: right">').append($('<input type="submit"  class="btn btn-outline-secondary" value="상세보기" style="padding-top: 2px; padding-bottom: 2px;">')));
                thead.append(thtr);
                table.append(thead);
            ...
            var allLoc = new kakao.maps.LatLng(36.80955273370213, 127.78850008427958);
            map.setCenter(allLoc);
            map.setLevel(13);
          },
          error: function(error) {
